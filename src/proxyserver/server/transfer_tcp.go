...
func sendLoginFailedToClient(conn net.Conn) {
	...
	_, _ = amf.Encode(&buffer, []int{0, 100})

	header.Length = uint32(buffer.Len()) + 12

	_ = binary.Write(conn, binary.BigEndian, &header)
